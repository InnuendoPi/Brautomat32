...
O InnuCase está disponível em dois tamanhos. Os arquivos de impressão são armazenados na pasta Housing/InnuCase.

## Formato da caixa

* InnuCase #1
    > uma caixa maior com display, dois módulos PWM e interruptor de alimentação\
    > Interruptor de alimentação para um microcontrolador ESP de fonte de alimentação externa\
    > 1x pressostato LED frontal M12, travamento
    > 2x M12 frontais para atuadores\
    > 2x fundo M12 para atuadores\
    > 3x fundo M12 para sensores\
    > Dimensões da caixa LxA 26x9cm\
    > Nome do arquivo: InnuCase-1_Display_2PWM_Power.stl\
    ![InnuCase #1](/docs/img/Case-1_Display_2PWM_Power.jpg)\
    > mesma caixa com display e dois módulos PWM sem interruptor de alimentação\
    > 2x M12 frontais para atuadores\
    > 2x fundo M12 para atuadores\
    > 3x fundo M12 para sensores\
    > Fonte de alimentação microcontrolador ESP via GGM IDS\
    > Dimensões da caixa LxA 26x9cm\
    > Nome do arquivo: InnuCase-1_Display_2PWM.stl\
    ![InnuCase #1](/docs/img/Case-1_Display_2PWM.jpg)

* InnuCase #1 com módulo PWM
...
*InnuCase #2
    > uma caixa menor com display\
    > 1x fundo M12 para sensor\
    > Dimensões da caixa LxA 20x9cm\
    > Nome do arquivo: InnuCase-2\
...
* InnuCase #1
    > uma caixa maior com display e dois módulos PWM\
    > Dimensões da tampa LxA 26x9cm\
    > Nome do arquivo: InnuCase-1\
    ![InnuCase #1](/docs/img/Deckel-1_Display_2PWM.jpg)
*InnuCase #2
    > uma caixa menor com display\
    > Dimensões da tampa LxA 20x9cm\
    > Nome do arquivo: InnuCase-2\
