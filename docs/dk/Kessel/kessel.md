...
## 🔧 Boiler control – direct vs. hybrid

Depending on the structure, the Brautomat can be operated with **1 to 3 boilers**. Each boiler controls its own hob via a GPIO.  
The Brautomat is designed to automatically work stably in all environments.

### ⚙️ Two possible control types

| Variant | Description | Advantages | Notes |
|-----------|---------------|-----------|-----------|
| **Direct control** | The command to the hob is carried out directly. All signals are sent directly to the output from the main program (e.g. `loop()` or ticker). | • Simplest structure<br>• No additional memory required<br>• Maximum speed | • Blocks the CPU briefly (approx. 5 ms per command)<br>• Not suitable for several boilers at the same time |
| **Hybrid control** | Commands are passed to a separate task via an internal **queue**. The task processes all commands one after the other. | • Stable with multiple boilers<br>• No signal collisions<br>• Background processing without CPU blocking | • Minimal additional memory requirement (~4 kB)<br> |

### 💡 Implementation

The Brautomat uses **Hybrid control**. This can be used universally and works inall environments, especially in environments with 2 or 3 induction hobs:

| Environment | Recommended variant | Justification |
|-----------|---------------------|-------------|
| **1 boiler** | Hybrid | same function as direct mode, without disadvantage |
| **2 boilers** | Hybrid | secure buffering of commands |
| **3 boilers** | Hybrid | prevents signal overlay |

> 🔸 Direct control is only suitable for special, very simple setups (e.g. single boilers with minimal code).  
>
> 🔹 The hybrid solution is the **standard and recommended variant** for all modern ESP32 assemblies.

### 📊 Technical comparison

| Feature | **Direct** | **Hybrid** |
|----------|------------|-------------|
| Memory usage | ~0 kB | ~4 kB |
| CPU blocking | yes (approx. 5 ms) | no |
| Stability (multiple boilers) | restricted | high |
| Timing Security | high (1 kettle) | very high (1–3 boilers) |
| Expandability | low | very good |
| Recommended Use | simple individual systems | universal multi-boiler systems |

---

### 🔍 Summary

Hybrid control is the **recommended and default activated mode**.  
It ensures that all signals are transmitted in a coordinated, secure and timely manner, even with multiple boilers -  
without blocking the main process.

> ✅ **In short:**  
>
> * For **1 boiler**: no disadvantage, same performance  
> * For **2-3 boilers**: full stability  
> * One software for all variants
